    ...
    try:
        if db.query(Level).count() == 0:
            ...
            for level_name, unit_titles in DEFAULT_UNITS.items():
                ...
                for index, unit_title in enumerate(unit_titles):
                    unit = Unit(
                        title=unit_title,
                    ...
                    for topic_index, topic_title in enumerate(_get_sample_topics(unit_title)):
                        topic = Topic(
                            title=topic_title,
...
def _get_sample_topics(unit_title: str) -> list[str]:
    topics_map = {
    ...
    return topics_map.get(
        unit_title,
        ["Introduction", "Core Concepts", "Advanced Topics", "Practice Questions", "Exam Preparation"],
    )
