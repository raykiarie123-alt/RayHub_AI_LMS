...
function StatCard({ icon: Icon, label, value, color, onClick }) {
  return (
    <div
      onClick={onClick}
      className={`bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 p-5 shadow-sm cursor-pointer hover:shadow-md transition-all ${
        onClick ? "hover:border-indigo-200 dark:hover:border-indigo-700" : ""
      }`}
    >
...
function QuickActionCard({ icon: Icon, label, description, onClick, color }) {
  return (
    <button
      onClick={onClick}
      className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 p-5 shadow-sm hover:shadow-md hover:border-indigo-200 dark:hover:border-indigo-700 transition-all text-left w-full"
    >
      <div className={`inline-flex items-center justify-center w-10 h-10 rounded-lg mb-3 ${color}`}>
        <Icon size={20} className="text-white" />
      </div>
...
function ActivityItem({ activity }) {
  const getIcon = () => {
    switch (activity.type) {
      ...
      case "quiz_attempt":
        return <Brain size={16} className="text-indigo-500" />;
      case "resource_added":
        return <Upload size={16} className="text-amber-500" />;
      case "plan_created":
    ...
    recentActivity?.length > 0
      ...
      : [
          ...
          },
          {
            ...
            description: "Audit Standards PDF",
            time: "1d ago",
          },
        ];
    ...
    <Layout title="Dashboard">
      <div className="mb-6">
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-1">
          Continue your CPA learning journey
        </p>
      </div>
        ...
        <StatCard
          ...
          value={
            summary?.total_study_time_minutes
              ? `${Math.round(summary.total_study_time_minutes / 60)}h`
              : "0h"
          }
          color="bg-amber-500"
        />
        ...
        <div>
          ...
          {currentPlan ? (
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 p-5 shadow-sm">
              ...
              <button
                onClick={() => navigate("/study-plan")}
                className="w-full px-4 py-2 bg-indigo-50 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 rounded-lg text-sm font-medium hover:bg-indigo-100 dark:hover:bg-indigo-900 transition-colors"
              >
                View Plan
              </button>
            </div>
          ) : (
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 p-5 shadow-sm text-center">
